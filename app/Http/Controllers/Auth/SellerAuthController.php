...
class SellerAuthController extends Controller
{
            ...
            if($seller){
                ...
                if (Auth::guard("seller")->attempt($credentials)) {
                    ...
                    if($request->remember == "1"){
                        Seller::where("id",$sellerId)->update(["remember_me"=>"1"]);
                    }
                    // if($seller->status != 1){
                    ...
                    // }
                    return response()->json(["success" => true, "message" => "Seller logged in successfully","url"=> route("seller.dashboard"), "id"=> $sellerId], 200);
                }
                else{
    ...
    public function postRegistration(Request $request){
        ...
        try{
            ...
            $register->save(); 
            if($register){
                if (!is_dir(storage_path("app/public/upload/sellers/"))) {
                    mkdir(storage_path("app/public/upload/sellers/"), 0755, true);
                }
                if($request->hasFile('image')){
                    $images = $request->file('image');
                    $img_name = Str::random(32).'.'.$images->getClientOriginalExtension();
                    $img_path = $request->file('image')->storeAs('upload/sellers', $img_name,'public');
                ...
                //     ]);
                // }
                return response()->json(["success" => true, "message" => "Congratulations, you have Successfully registered","url"=> route("seller.login"), "id"=> $register->id], 200);
            } else{
    ...
    public function dashboard(){
        if(Auth::guard('seller')->check()){
            ...
            $seller = Seller::where("id", $id)->whereNull("deleted_at")->first();
            if(!$seller){
